...
@dataclass
class Pattern:
    name: str; definition: Dict[str,Any]; discovery_range: Dict[str,Any]; symbols: List[str]; status: PatternStatus=PatternStatus.DISCOVERED; discovered_at_ms:int=field(default_factory=lambda:int(time.time()*1000)); sample_size:int=0; discovery_result:Dict[str,Any]=field(default_factory=dict); oos_result:Dict[str,Any]=field(default_factory=dict); walk_forward_result:Dict[str,Any]=field(default_factory=dict); live_forward_result:Dict[str,Any]=field(default_factory=dict)
    @classmethod
    def create(cls, name, definition, discovery_range, symbols): return cls(name, definition, discovery_range, list(symbols))
    def mark_backtested(self, sample_size:int, result:Dict[str,Any]): self.sample_size=sample_size; self.discovery_result=dict(result); self.status=PatternStatus.BACKTESTED
