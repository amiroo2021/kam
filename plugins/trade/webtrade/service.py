"""TradeDesk-backed service layer for WebTrade (read + write via TradeDesk)."""
...
class WebTradeService:
    ...
    def __init__(
        ...
        session_secret: str = "webtrade-dev-secret",
    ) -> None:
    ...
    def _execute_write(
        ...
        if resp.success:
            ...
            logger.info(
                "WebTrade write ok op=%s exchange=%s account=%s symbol=%s ms=%.1f",
                operation,
                exchange,
                account,
            ...
            out["error"] = self._safe_error(resp)
            logger.info(
                "WebTrade write fail op=%s exchange=%s account=%s symbol=%s code=%s",
                operation,
                exchange,
                account,
                (extra or {}).get("symbol") or "",
    ...
    def preview_order(
        ...
        if ot != "limit":
            ...
            return {
                "success": False,
                "error": {"code": "INVALID_ORDER_TYPE", "message": "Only LIMIT orders are supported via WebTrade."},
            }
        caps = set(self.desk.capabilities(exchange))
    ...
    def execute_preview(self, preview_id: str) -> Dict[str, Any]:
        ...
        if kind == "order":
            ...
            if resp.success:
                ...
                logger.info(
                    "WebTrade order ok exchange=%s account=%s symbol=%s side=%s",
                    exchange,
                    account,
        ...
        if kind == "ladder":
            ...
            if resp.success:
                logger.info(
                    "WebTrade ladder ok exchange=%s account=%s symbol=%s children=%s",
                    exchange,
                    account,
                    plan.get("native_symbol"),
