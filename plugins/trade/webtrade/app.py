"""WebTrade FastAPI application — discovery + chart + positions/orders + writes."""
...
from .config import WebTradeConfig, WebTradeConfigError, load_config
from .marketdata import SUPPORTED_TFS, fetch_candles
from .service import WebTradeService

logger = logging.getLogger("webtrade")
STATIC_DIR = Path(__file__).resolve().parent / "static"


def create_app(
    config: Optional[WebTradeConfig] = None,
    service: Optional[WebTradeService] = None,
) -> FastAPI:
    try:
        cfg = config or load_config()
    except WebTradeConfigError:
        # Fail-closed app: only health-style error page, no data APIs.
        app = FastAPI(title="webtrade", version="0.2.0")
    ...
    svc = service or WebTradeService(session_secret=cfg.session_secret)
  ...
  <form class="login-card" method="post" action="/login" autocomplete="current-password">
    ...
    def _app_page() -> HTMLResponse:
        ...
        if path.is_file():
            ...
            return resp
        return HTMLResponse("<h1>webtrade</h1><p>Static UI missing.</p>", status_code=500)
    ...
    async def login_post(request: Request, password: str = Form(...)) -> Response:
        ...
        if not sessions.password_ok(password):
            ...
            logger.info("webtrade login failure from %s", key)
    ...
    async def logout() -> Response:
        ...
        resp.delete_cookie("webtrade_csrf", path="/")
        return resp

    @app.get("/api/health")
    async def health() -> dict:
        return {"ok": True, "service": "webtrade", "phase": 2}
...
def main() -> None:
    ...
    cfg = load_config()
    uvicorn.run(
        "plugins.trade.webtrade.app:app",
        host=cfg.host,
        port=cfg.port,
        reload=False,
        log_level="info",
