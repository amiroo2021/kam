"""WebBacktest configuration — reads WEB_* from environment / Hermes .env."""
...
class WebBacktestConfigError(RuntimeError):
    """Raised when WebBacktest cannot start safely (fail closed)."""
...
class WebBacktestConfig:
    def __init__(self) -> None:
        ...
        if not password:
            raise WebBacktestConfigError(
                "WEB_PASSWORD is missing. WebBacktest refuses to start unprotected."
            )
        self.password = password
        self.hint = _read_env("WEB_HINT") or "Contact the operator for access."
        secret = _read_env("WEBBACKTEST_SESSION_SECRET")
        if not secret:
            secret = secrets.token_urlsafe(48)
            _persist_env_key("WEBBACKTEST_SESSION_SECRET", secret)
            secret = _read_env("WEBBACKTEST_SESSION_SECRET") or secret
        self.session_secret = secret
        self.cookie_name = "webbacktest_session"
