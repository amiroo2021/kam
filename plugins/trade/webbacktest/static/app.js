/* webbacktest chart client — RENDERS backend state only. No ladder math. */
(function () {
  ...
  let currentMode = "REPLAY_TO_LIVE"; // phase 1 webbacktest: start-to-live
  let serverMode = "LIVE";
  let lastPhase = "";
  let lastProgress = {};
  ...
  function syncModeUi() {
    ...
    // Phase 1: start-only WebBacktest.
    const needStart = currentMode === "BACKTEST" || currentMode === "REPLAY_TO_LIVE";
    const needEnd = false;
    const startWrap = document.getElementById("startWrap");
    if (startWrap) startWrap.hidden = !needStart;
    const endWrap = document.getElementById("endWrap");
    if (endWrap) endWrap.hidden = !needEnd;
    document.getElementById("applyBtn").textContent = "START BACKTEST";
    refreshConnectionStatus();
  }
